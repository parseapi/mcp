...
export function buildServer(key: string | null, transport: Transport, options: { mode?: CatalogMode } = {}): McpServer {
	...
	tool(
		...
		'Find company directory candidates using at most one of query, domain, ticker or identifier, or discover by country, an exact industry pair or selected registration authority. Registration form/status are exact source values; administrative status does not establish current business activity. Returns companies and an opaque next cursor without choosing a match. Deep belongs to each result. Keep the same selector, filters and limit when sending cursor. Empty listings do not establish private ownership.',
		{
			...
			// The API bounds UTF-16 units; Zod maxLength counts Unicode code points.
			registration_form: z.string().min(1).max(200).refine(value => value.length <= 200 && value.trim().length > 0 && !/\p{Cc}/u.test(value), 'Use a nonblank source string within the API length limit and without control characters.').optional().describe('Exact case-sensitive source legal-form code; requires registration_authority. DPC does not establish public/private ownership; DNC does not establish tax exemption'),
			registration_status: z.string().min(1).max(200).refine(value => value.length <= 200 && value.trim().length > 0 && !/\p{Cc}/u.test(value), 'Use a nonblank source string within the API length limit and without control characters.').optional().describe('Exact case-sensitive administrative source status, such as Good Standing; requires registration_authority; does not establish trading, solvency or present business existence'),
