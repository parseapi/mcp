...
async function smokeStdio() {
	...
	check('stdio tools/list has 37 tools', names.length === 37, `got ${names.length}`);
...
async function smokeHttp() {
	...
	try {
		...
		const names = (list.result?.tools ?? []).map((t) => t.name);
		check('http tools/list has 36 tools (no ip_self)', names.length === 36 && !names.includes('ip_self'), `got ${names.length}`);
